...
1. radio, checkbox와 **형제관계**인 태그 준비 (예) label, span 등
2. 1번 형제태그에 background-image로 radio, checkbox를 대신한 이미지 적용
3. `input:checked + 배경이미지가 들어간 형제태그 {}` 선택자 준비
4. 위 3번 선택자에 선택됐을 때 변경 될 배경이미지 재연결
5. 1번 태그 선택자에 `display:none` 적용으로 기존 input 숨기기
### option, select 디자인 방법
* select 대신 div 태그 사용해서 디자인 하기(호랑가시나무 객실선택 참고)
